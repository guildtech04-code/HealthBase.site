-- ============================================
-- EHR Module Database Schema - MANUAL VERSION
-- For MariaDB compatibility - run each section separately
-- ============================================
...
-- 6. Add service_id column to appointments table (MANUAL - Check if exists first)
...
-- 7. Add index for service_id (Run only if index doesn't exist)
ALTER TABLE `appointments` ADD INDEX `idx_appt_service` (`service_id`);

-- 8. Add foreign key for service_id (only if FK doesn't exist)
-- Note: You may need to drop existing FK first if any
